  ...
  form.addEventListener('submit', async function (event) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        formStatus.innerHTML = 'Submission successful! Your message has been received.';

        // Reset the form
        form.reset();

        // Add approval logic (client-side for now)
        setTimeout(() => {
          formStatus.innerHTML += '<br>Status: Approved!';
          formStatus.style.color = 'green';
        }, 2000); // Simulate approval after 2 seconds
      } else {
        // Submission failed
        formStatus.style.color = 'red';
        formStatus.innerHTML = 'Error submitting the form. Please try again.';
      }
    } catch (error) {
      // Handle network errors
      formStatus.style.color = 'red';
      formStatus.innerHTML = 'Error: Could not connect to the server. Please try again later.';
